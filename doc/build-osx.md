...
This guide describes how to build yonad, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build Yona Core from source.
...
### 4. Clone Yona repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the Yona Core repository to a directory.
All build scripts and commands will run from this directory.

``` bash
git clone --recursive https://github.com/yonaproject/yona.git
```
...
It is not necessary to build wallet functionality to run `yonad` or  `yona-qt`.
...
Yona Core includes a GUI built with the cross-platform Qt Framework.
...
You can deploy a `.dmg` containing the Yona Core application using `make deploy`.
...
There are many ways to configure Yona Core, here are a few common examples:
...
Run the following in your terminal to compile Yona Core:
...
Yona Core should now be available at `./src/yonad`.
If you compiled support for the GUI, it should be available at `./src/qt/yona-qt`.

The first time you run `yonad` or `yona-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/Yona/
```
...
./src/qt/yona-qt -server # Starts the yona-qt server mode, allows yona-cli control
```

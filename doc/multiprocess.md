...
On unix systems, the `--enable-multiprocess` build option can be passed to `./configure` to build new `yona-node`, `yona-wallet`, and `yona-gui` executables alongside existing `yonad` and `yona-qt` executables.

`yona-node` is a drop-in replacement for `yonad`, and `yona-gui` is a drop-in replacement for `yona-qt`, and there are no differences in use or external behavior between the new and old executables. But internally (after [#10102](https://github.com/bitcoin/bitcoin/pull/10102)), `yona-gui` will spawn a `yona-node` process to run P2P and RPC code, communicating with it across a socket pair, and `yona-node` will spawn `yona-wallet` to run wallet code, also communicating over a socket pair. This will let node, wallet, and GUI code run in separate address spaces for better isolation, and allow future improvements like being able to start and stop components independently on different machines and environments.

## Next steps

Specific next steps after [#10102](https://github.com/bitcoin/bitcoin/pull/10102) will be:

- [ ] Adding `-ipcbind` and `-ipcconnect` options to `yona-node`, `yona-wallet`, and `yona-gui` executables so they can listen and connect to TCP ports and unix socket paths. This will allow separate processes to be started and stopped any time and connect to each other.
- [ ] Adding `-server` and `-rpcbind` options to the `yona-wallet` executable so wallet processes can handle RPC requests directly without going through the node.
...
make
src/yona-node -regtest -printtoconsole -debug=ipc
BITCOIND=yona-node test/functional/test_runner.py
```

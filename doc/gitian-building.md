Gitian building
================
*Setup instructions for a Gitian build of Yona Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Yona
...
- [Building Yona Core](#building-yona-core)
...
Clone the git repositories for yona and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/yonaproject/yona --recursive
git clone https://github.com/yonaproject/gitian.sigs.git
```
...
```bash
sudo cp /home/debian/yona/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the yona repository under 'Fetch and create inputs' to install sources which require
...
To build Yona Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the yona repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/yona/.git/
...
For example:
```bash
URL=https://github.com/laanwj/yona.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit yona=${COMMIT} --url yona=${URL} ../yona/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit yona=${COMMIT} --url yona=${URL} ../yona/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit yona=${COMMIT} --url yona=${URL} ../yona/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-yona:
./bin/gbuild --commit yona=${COMMIT},cpp-eth-yona=${ETHCOMMIT} --url yona=${URL},cpp-eth-yona=${ETHURL} ../yona/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the yona git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../yona/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/yonaproject/yona-detached-sigs.git

BTCPATH=/some/root/path/yona
SIGPATH=/some/root/path/yona-detached-sigs

./bin/gbuild --url yona=${BTCPATH},signature=${SIGPATH} ../yona/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/yona-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/yona-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/yona-osx-build.assert
```
...
[yonaproject/gitian.sigs](https://github.com/yonaproject/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@www.yonachain.com) and he will commit them.

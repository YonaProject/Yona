...
#include <qt/yonaversionchecker.h>
    ...
    m_network_style(networkStyle)
{
    ...
    yonaVersionChecker = new YonaVersionChecker(this);
...
void BitcoinGUI::createActions()
{
    ...
    YRCTokenAction = new QAction(platformStyle->MultiStatesIcon(":/icons/yrctoken"), tr("&YRC Tokens"), this);
    YRCTokenAction->setStatusTip(tr("YRC Tokens (send, receive or add Tokens in list)"));
    YRCTokenAction->setToolTip(YRCTokenAction->statusTip());
    YRCTokenAction->setCheckable(true);
    YRCTokenAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_6));
    tabGroup->addAction(YRCTokenAction);
    ...
    connect(YRCTokenAction, SIGNAL(triggered()), this, SLOT(showNormalIfMinimized()));
    connect(YRCTokenAction, SIGNAL(triggered()), this, SLOT(gotoTokenPage()));
    ...
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Yona Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Yona Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a yona: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Yona command-line options").arg(PACKAGE_NAME));
...
void BitcoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        appNavigationBar->addAction(YRCTokenAction);
...
void BitcoinGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    {
        // Check for updates
        if(_clientModel->getOptionsModel()->getCheckForUpdates() && yonaVersionChecker->newVersionAvailable())
        {
            QString link = QString("<a href=%1>%2</a>").arg(YONA_RELEASES, YONA_RELEASES);
            QString message(tr("New version of Yona wallet is available on the Yona source code repository: <br /> %1. <br />It is recommended to download it and update this application").arg(link));
...
void BitcoinGUI::setWalletActionsEnabled(bool enabled)
{
    ...
    YRCTokenAction->setEnabled(enabled);
...
{
    YRCTokenAction->setChecked(true);
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Yona network.", "", count);
    } else {
        //: A substring of the tooltip.

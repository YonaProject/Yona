...
class YonaHwiToolPriv
{
public:
    YonaHwiToolPriv(QObject *parent)
    {
...
YonaHwiTool::YonaHwiTool(QObject *parent) : QObject(parent)
{
    d = new YonaHwiToolPriv(this);
}

YonaHwiTool::~YonaHwiTool()
{
    delete d;
}

bool YonaHwiTool::enumerate(QList<HWDevice> &devices, bool stake)
{
    LOCK(cs_ledger);
    devices.clear();
    std::vector<LedgerDevice> vecDevices;
    if(YonaLedger::instance().enumerate(vecDevices, stake))
    {
    ...
    {
        d->strError = QString::fromStdString(YonaLedger::instance().errorMessage());
    }

    return devices.size() > 0;
}

bool YonaHwiTool::isConnected(const QString &fingerprint, bool stake)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    bool ret = YonaLedger::instance().isConnected(strFingerprint, stake);
    if(!ret) d->strError = QString::fromStdString(YonaLedger::instance().errorMessage());
    return ret;
}

bool YonaHwiTool::getKeyPool(const QString &fingerprint, int type, const QString& path, bool internal, QString &desc)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    ...
    bool ret = YonaLedger::instance().getKeyPool(strFingerprint, type, strPath, internal, d->from, d->to, strDesc);
...
bool YonaHwiTool::getKeyPool(const QString &fingerprint, int type, const QString &path, QStringList &descs)
{
    LOCK(cs_ledger);
    bool ret = true;
    QString desc;
...
bool YonaHwiTool::getKeyPoolPKH(const QString &fingerprint, const QString& path, QStringList &descs)
{
    return getKeyPool(fingerprint, (int)OutputType::LEGACY, path, descs);
}

bool YonaHwiTool::getKeyPoolP2SH(const QString &fingerprint, const QString& path, QStringList &descs)
{
    return getKeyPool(fingerprint, (int)OutputType::P2SH_SEGWIT, path, descs);
}

bool YonaHwiTool::getKeyPoolBech32(const QString &fingerprint, const QString& path, QStringList &descs)
{
    return getKeyPool(fingerprint, (int)OutputType::BECH32, path, descs);
}

bool YonaHwiTool::signTx(const QString &fingerprint, QString &psbt)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    std::string strPsbt = psbt.toStdString();
    bool ret = YonaLedger::instance().signTx(strFingerprint, strPsbt);
    psbt = QString::fromStdString(strPsbt);
    if(!ret) d->strError = QString::fromStdString(YonaLedger::instance().errorMessage());
    return ret;
}

bool YonaHwiTool::signMessage(const QString &fingerprint, const QString &message, const QString &path, QString &signature)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    ...
    bool ret = YonaLedger::instance().signMessage(strFingerprint, strMessage, strPath, strSignature);
    signature = QString::fromStdString(strSignature);
    if(!ret) d->strError = QString::fromStdString(YonaLedger::instance().errorMessage());
    return ret;
}

bool YonaHwiTool::signDelegate(const QString &fingerprint, QString &psbt)
{
...
QString YonaHwiTool::errorMessage()
{
...
bool YonaHwiTool::isStarted()
{
    return d->fStarted;
}

void YonaHwiTool::wait()
{
    if(d->fStarted)
    {
...
bool YonaHwiTool::rescanBlockchain(int startHeight, int stopHeight)
{
    if(!d->model) return false;
...
bool YonaHwiTool::importAddresses(const QString &desc)
{
    if(!d->model) return false;
...
bool YonaHwiTool::importMulti(const QStringList &descs)
{
    bool ret = true;
...
bool YonaHwiTool::finalizePsbt(const QString &psbt, QString &hexTx, bool &complete)
{
    if(!d->model) return false;
...
bool YonaHwiTool::sendRawTransaction(const QString &hexTx, QVariantMap& variantMap)
{
    if(!d->model) return false;
...
bool YonaHwiTool::decodePsbt(const QString &psbt, QString &decoded)
{
    if(!d->model) return false;
...
void YonaHwiTool::setModel(WalletModel *model)
{
    d->model = model;
}

bool YonaHwiTool::execRPC(ExecRPCCommand *cmd, const QMap<QString, QString> &lstParams, QVariant &result, QString &resultJson)
{
    d->strError.clear();
...
void YonaHwiTool::addError(const QString &error)
{
    if(d->strError != "")
        d->strError += "\n";
    d->strError += error;
...
QString YonaHwiTool::derivationPathPKH()
{
    std::string path = YonaLedger::instance().derivationPath((int)OutputType::LEGACY);
    return QString::fromStdString(path);
}

QString YonaHwiTool::derivationPathP2SH()
{
    std::string path = YonaLedger::instance().derivationPath((int)OutputType::P2SH_SEGWIT);
    return QString::fromStdString(path);
}

QString YonaHwiTool::derivationPathBech32()
{
    std::string path = YonaLedger::instance().derivationPath((int)OutputType::BECH32);
    return QString::fromStdString(path);
}

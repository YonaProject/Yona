...
#define paternVersion "yona-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

YonaVersionChecker::YonaVersionChecker(QObject *parent) : QObject(parent)
{
    currentVersion = Version(CLIENT_VERSION_MAJOR, CLIENT_VERSION_MINOR, 0);
}

YonaVersionChecker::~YonaVersionChecker()
{

}

bool YonaVersionChecker::newVersionAvailable()
{
    Version maxReleaseVersion = getMaxReleaseVersion();
    return maxReleaseVersion > currentVersion;
}

QList<Version> YonaVersionChecker::getVersions()
{
    QNetworkAccessManager manager;
    QNetworkReply *response = manager.get(QNetworkRequest(QUrl(YONA_RELEASES)));
    QTimer timer;
...
Version YonaVersionChecker::getMaxReleaseVersion()
{

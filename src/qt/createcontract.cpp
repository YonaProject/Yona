...
static const CAmount SINGLE_STEP = 0.000000001*COIN;
    ...
    m_results(1)
{
    ...
    ui->labelSenderAddress->setToolTip(tr("The yona address that will be used to create the contract."));
...
void CreateContract::setModel(WalletModel *_model)
{
    ...
    if (bCreateUnsigned) {
        ...
        ui->pushButtonCreateContract->setToolTip(tr("Creates a Partially Signed Yona Transaction (PSBT) for use with e.g. an offline %1 wallet, or a PSBT-compatible hardware wallet.").arg(PACKAGE_NAME));
    }
}
...
void CreateContract::on_createContractClicked()
{
    if(isDataValid())
    {
        ...
        if (bCreateUnsigned) {
            ...
            questionString.append(tr("This will produce a Partially Signed Yona Transaction (PSBT) which you can copy and then sign with e.g. an offline %1 wallet, or a PSBT-compatible hardware wallet.").arg(PACKAGE_NAME));

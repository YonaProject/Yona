    ...
    m_execRPCCommand(nullptr)
{
    ...
    ui->lineEditGasLimit->setMinimum(DEFAULT_GAS_LIMIT_OP_CREATE);
    ui->lineEditGasLimit->setMaximum(ADD_DELEGATION_MIN_GAS_LIMIT);
    ui->lineEditGasLimit->setValue(ADD_DELEGATION_MIN_GAS_LIMIT);
...
void AddDelegationPage::setModel(WalletModel *_model)
{
    ...
    if (bCreateUnsigned) {
        ...
        ui->addDelegationButton->setToolTip(tr("Creates a Partially Signed Yona Transaction (PSBT) for use with e.g. an offline %1 wallet, or a PSBT-compatible hardware wallet.").arg(PACKAGE_NAME));
    }
}
...
void AddDelegationPage::on_gasInfoChanged(quint64 blockGasLimit, quint64 minGasPrice, quint64 nGasPrice)
{
    ...
    ui->labelGasPrice->setToolTip(tr("Gas price: YONA price per gas unit. Default = %1, Min = %2").arg(QString::fromStdString(FormatMoney(DEFAULT_GAS_PRICE))).arg(QString::fromStdString(FormatMoney(minGasPrice))));
...
void AddDelegationPage::on_addDelegationClicked()
{
    if(m_model)
    {
        ...
        if (bCreateUnsigned) {
            ...
            questionString.append(tr("Please, review your transaction proposal. This will produce a Partially Signed Yona Transaction (PSBT) which you can copy and then sign with e.g. an offline %1 wallet, or a PSBT-compatible hardware wallet.").arg(PACKAGE_NAME));

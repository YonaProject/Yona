...
Token::Token()
{
    ...
    lstMandatory.append(YonaToken::paramAddress());
    lstMandatory.append(YonaToken::paramDatahex());
    QStringList lstOptional;
    lstOptional.append(YonaToken::paramSender());
    ...
    lstMandatory.append(YonaToken::paramDatahex());
    lstOptional.clear();
    lstOptional.append(YonaToken::paramAmount());
    lstOptional.append(YonaToken::paramGasLimit());
    lstOptional.append(YonaToken::paramGasPrice());
    lstOptional.append(YonaToken::paramSender());
    lstOptional.append(YonaToken::paramBroadcast());
    lstOptional.append(YonaToken::paramChangeToSender());
    lstOptional.append(YonaToken::paramPsbt());
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            if(numTopics > 1)
            {
                ...
                Token::ToYonaAddress(tokenEvent.sender, tokenEvent.sender);
            }
            if(numTopics > 2)
            {
                tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
                Token::ToYonaAddress(tokenEvent.receiver, tokenEvent.receiver);
            }

...
#include <yonatests/test_utils.h>
...
BOOST_AUTO_TEST_CASE(bytecodeexec_txs_empty){
    initState();
    std::vector<YonaTransaction> txs;
...
BOOST_AUTO_TEST_CASE(bytecodeexec_create_contract){
    initState();
    YonaTransaction txEth = createYonaTransaction(CODE[0], 0, GASLIMIT, dev::u256(1), HASHTX, dev::Address());
    std::vector<YonaTransaction> txs(1, txEth);
    auto result = executeBC(txs, *m_node.chainman);
    
    std::vector<dev::Address> addrs = {createYonaAddress(txs[0].getHashWith(), txs[0].getNVout())};
...
BOOST_AUTO_TEST_CASE(bytecodeexec_create_contract_OutOfGasIntrinsic){
    initState();
    YonaTransaction txEth = createYonaTransaction(CODE[0], 0, dev::u256(100), dev::u256(1), HASHTX, dev::Address());
    std::vector<YonaTransaction> txs(1, txEth);
    auto result = executeBC(txs, *m_node.chainman);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_create_contract_OutOfGas){
    initState();
    YonaTransaction txEth = createYonaTransaction(CODE[1], 0, GASLIMIT, dev::u256(1), HASHTX, dev::Address());
    std::vector<YonaTransaction> txs(1, txEth);
    auto result = executeBC(txs, *m_node.chainman);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_OutOfGasIntrinsic_create_contract_normal_create_contract){
    ...
    for(size_t i = 0; i < 10; i++){
        ...
        YonaTransaction txEth = createYonaTransaction(CODE[0], 0, gasLimit, dev::u256(1), hash, dev::Address());
        newAddressGen.push_back(createYonaAddress(txEth.getHashWith(), txEth.getNVout()));
        txs.push_back(txEth);
        ++hash;
    }
    auto result = executeBC(txs, *m_node.chainman);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_OutOfGas_create_contract_normal_create_contract){
    ...
    for(size_t i = 0; i < 10; i++){
        ...
        YonaTransaction txEth = createYonaTransaction(code, 0, GASLIMIT, dev::u256(1), hash, dev::Address());
        newAddressGen.push_back(createYonaAddress(txEth.getHashWith(), txEth.getNVout()));
        txs.push_back(txEth);
        ++hash;
    }
...
BOOST_AUTO_TEST_CASE(bytecodeexec_create_contract_many){
    ...
    for(size_t i = 0; i < 130; i++){
        YonaTransaction txEth = createYonaTransaction(CODE[0], 0, GASLIMIT, dev::u256(1), hash, dev::Address(), i);
        newAddressGen.push_back(createYonaAddress(txEth.getHashWith(), txEth.getNVout()));
        txs.push_back(txEth);
        ++hash;
    }
...
BOOST_AUTO_TEST_CASE(bytecodeexec_call_contract_transfer){
    initState();
    YonaTransaction txEthCreate = createYonaTransaction(CODE[0], 0, GASLIMIT, dev::u256(1), HASHTX, dev::Address());
    std::vector<YonaTransaction> txsCreate(1, txEthCreate);
    executeBC(txsCreate, *m_node.chainman);
    std::vector<dev::Address> addrs = {createYonaAddress(txsCreate[0].getHashWith(), txsCreate[0].getNVout())};
    YonaTransaction txEthCall = createYonaTransaction(ParseHex("00"), 1300, GASLIMIT, dev::u256(1), HASHTX, addrs[0]);
    std::vector<YonaTransaction> txsCall(1, txEthCall);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_call_contract_transfer_OutOfGasIntrinsic_return_value){
    ...
    std::vector<YonaTransaction> txsCreate(1, txEthCreate);
    executeBC(txsCreate, *m_node.chainman);
    dev::Address newAddress(createYonaAddress(txsCreate[0].getHashWith(), txsCreate[0].getNVout()));
    YonaTransaction txEthCall = createYonaTransaction(ParseHex("00"), 1300, dev::u256(1), dev::u256(1), HASHTX, newAddress);
    std::vector<YonaTransaction> txsCall(1, txEthCall);
    auto result = executeBC(txsCall, *m_node.chainman);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_call_contract_transfer_OutOfGas_return_value){
    initState();
    YonaTransaction txEthCreate = createYonaTransaction(CODE[2], 0, GASLIMIT, dev::u256(1), HASHTX, dev::Address());
    std::vector<YonaTransaction> txsCreate(1, txEthCreate);
    executeBC(txsCreate, *m_node.chainman);
    dev::Address newAddress(createYonaAddress(txsCreate[0].getHashWith(), txsCreate[0].getNVout()));
    YonaTransaction txEthCall = createYonaTransaction(ParseHex("00"), 1300, GASLIMIT, dev::u256(1), HASHTX, newAddress);
    std::vector<YonaTransaction> txsCall(1, txEthCall);
    auto result = executeBC(txsCall, *m_node.chainman);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_call_contract_transfer_many){
    ...
    std::vector<YonaTransaction> txsCall;
    std::vector<dev::Address> addrs;
    valtype codeCall(ParseHex("00"));
    dev::h256 hashCall(HASHTX);
    for(size_t i = 0; i < txs.size(); i++){
        YonaTransaction txEthCall = createYonaTransaction(codeCall, 1300, GASLIMIT, dev::u256(1), hash, newAddressGen[i], i);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_call_contract_OutOfGas_transfer_many_return_value){
    ...
    dev::h256 hash(HASHTX);
    for(size_t i = 0; i < 130; i++){
        YonaTransaction txEth = createYonaTransaction(CODE[2], 0, GASLIMIT, dev::u256(1), hash, dev::Address(), i);
        newAddressGen.push_back(createYonaAddress(txEth.getHashWith(), txEth.getNVout()));
        txs.push_back(txEth);
        ++hash;
    }
    ...
    dev::h256 hashCall(HASHTX);
    for(size_t i = 0; i < txs.size(); i++){
        YonaTransaction txEthCall = createYonaTransaction(codeCall, 1300, GASLIMIT, dev::u256(1), hashCall, newAddressGen[i], i);
        txsCall.push_back(txEthCall);
        addrs.push_back(txEthCall.receiveAddress());
        ++hashCall;
    }
...
BOOST_AUTO_TEST_CASE(bytecodeexec_suicide){
    initState();
    std::vector<YonaTransaction> txsCreate;
    std::vector<dev::Address> newAddresses;
    dev::h256 hash(HASHTX);
    for(size_t i = 0; i < 10; i++){
        valtype code = i == 0 ? CODE[0] : CODE[4];
        dev::u256 value = i == 0 ? dev::u256(0) : dev::u256(13);
        YonaTransaction txEthCreate = createYonaTransaction(code, 0, GASLIMIT, dev::u256(1), hash, dev::Address(), i);
        txsCreate.push_back(txEthCreate);

        YonaTransaction txEthSendValue;
        if(i != 0){
            txEthSendValue = createYonaTransaction(valtype(), value, GASLIMIT, dev::u256(1), hash, createYonaAddress(txEthCreate.getHashWith(), txEthCreate.getNVout()), i);
            txsCreate.push_back(txEthSendValue);
        }

        newAddresses.push_back(createYonaAddress(txEthCreate.getHashWith(), txEthCreate.getNVout()));
        ++hash;
    }
    ...
    for(size_t i = 0; i < newAddresses.size() - 1; i++){
        YonaTransaction txEthCall = createYonaTransaction(codeCall, 0, GASLIMIT, dev::u256(1), HASHTX, newAddresses[i + 1]);
        txsCall.push_back(txEthCall);
        addrs.push_back(txEthCall.receiveAddress());
    }
    auto result = executeBC(txsCall, *m_node.chainman);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_contract_create_contracts){
    initState();
    YonaTransaction txEthCreate = createYonaTransaction(CODE[3], 0, GASLIMIT, dev::u256(1), HASHTX, dev::Address());
    std::vector<YonaTransaction> txs(1, txEthCreate);
    executeBC(txs, *m_node.chainman);
    std::vector<YonaTransaction> txsCall;
    valtype codeCall(ParseHex("3f811b80"));
    dev::Address newAddress(createYonaAddress(txEthCreate.getHashWith(), txEthCreate.getNVout()));
    std::vector<dev::Address> addrs;
    for(size_t i = 0; i < 20; i++){
        YonaTransaction txEthCall = createYonaTransaction(codeCall, 0, GASLIMIT, dev::u256(1), HASHTX, newAddress, i);
        txsCall.push_back(txEthCall);
        addrs.push_back(txEthCall.receiveAddress());
    }
    auto result = executeBC(txsCall, *m_node.chainman);

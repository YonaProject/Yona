...
void checkResult(bool isCreation, std::vector<YonaTransaction> results, uint256 hash){
...
void runTest(CChainState& chainstate, CTxMemPool& mempool, bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    YonaTxConverter converter(transaction, chainstate, &mempool, NULL);
    ExtractYonaTX yonaTx;
    BOOST_CHECK(converter.extractionYonaTransactions(yonaTx));
    std::vector<YonaTransaction> result = yonaTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(yonatxconverter_tests, TestingSetup)

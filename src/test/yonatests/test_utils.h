#ifndef YONATESTS_TEST_UTILS_H
#define YONATESTS_TEST_UTILS_H
...
inline void initState(){
    ...
    const std::string dirYona = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<YonaState>(new YonaState(dev::u256(0), YonaState::openDB(dirYona, hashDB, dev::WithExisting::Trust), dirYona + "/yonaDB", dev::eth::BaseState::Empty));
...
inline dev::Address createYonaAddress(dev::h256 hashTx, uint32_t voutNumber){
...
inline YonaTransaction createYonaTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout = 0){
    YonaTransaction txEth;
    if(recipient == dev::Address()){
        txEth = YonaTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = YonaTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
inline std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<YonaTransaction> txs, ChainstateManager& chainman){
    CBlock block(generateBlock());
    YonaDGP yonaDGP(globalState.get(), chainman.ActiveChainstate(), fGettingValuesDGP);
    uint64_t blockGasLimit = yonaDGP.getBlockGasLimit(chainman.ActiveChain().Tip()->nHeight + 1);

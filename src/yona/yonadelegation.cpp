...
bool YonaDelegation::GetDelegation(const uint160 &address, Delegation &delegation, CChainState& chainstate) const
{
...
bool YonaDelegation::VerifyDelegation(const uint160 &address, const Delegation &delegation)
{
...
bool YonaDelegation::FilterDelegationEvents(std::vector<DelegationEvent> &events, const IDelegationFilter &filter, ChainstateManager &chainman, int fromBlock, int toBlock, int minconf) const
{
...
std::map<uint160, Delegation> YonaDelegation::DelegationsFromEvents(const std::vector<DelegationEvent> &events)
{
...
void YonaDelegation::UpdateDelegationsFromEvents(const std::vector<DelegationEvent> &events, std::map<uint160, Delegation> &delegations)
{
...
bool YonaDelegation::ExistDelegationContract() const
{
    LOCK(cs_main);
...
std::string YonaDelegation::BytecodeRemove()
{
    return DelegationABI()["removeDelegation"].selector();
}

bool YonaDelegation::BytecodeAdd(const std::string &hexStaker, const int &fee, const std::vector<unsigned char> &PoD, std::string &datahex, std::string &errorMessage)
{
...
bool YonaDelegation::IsAddBytecode(const std::vector<unsigned char> &data)
{
...
bool YonaDelegation::GetUnsignedStaker(const std::vector<unsigned char> &data, std::string &hexStaker)
{
...
bool YonaDelegation::SetSignedStaker(std::vector<unsigned char> &data, const std::string &base64PoD)
{
    if(!IsAddBytecode(data))
        return false;

...
YonaState::YonaState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = YonaState::openDB(_path + "/yonaDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

YonaState::YonaState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute YonaState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, YonaTransaction const& _t, CChain& _chain, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createYonaAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            yona::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
    ...
    if(voutLimit){
        ...
        return ResultExecute{
            ex,
            YonaTransactionReceipt(oldStateRoot, oldUTXORoot, gas, e.logs(), {}, {}),
        ...
        if (res.excepted == dev::eth::TransactionException::None) {
            ...
            return ResultExecute{
                res,
                YonaTransactionReceipt(rootHash(), rootHashUTXO(), startGasUsed + e.gasUsed(), e.logs(), {}, {}),
                tx ? *tx : CTransaction()
            };
        }
    }
...
std::unordered_map<dev::Address, Vin> YonaState::vins() const // temp
{
...
void YonaState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* YonaState::vin(dev::Address const& _a) const
{
    return const_cast<YonaState*>(this)->vin(_a);
}

Vin* YonaState::vin(dev::Address const& _addr)
{
...
// void YonaState::commit(CommitBehaviour _commitBehaviour)
// {
...
void YonaState::kill(dev::Address _addr)
{
...
void YonaState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
void YonaState::deleteAccounts(std::set<dev::Address>& addrs){
...
void YonaState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void YonaState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
    clog(dev::VerbosityWarning, "exec") << "VM exception:" << ss.str();
}

void YonaState::validateTransfersWithChangeLog(){
...
void YonaState::deployDelegationsContract(){
    dev::Address delegationsAddress = uintToh160(Params().GetConsensus().delegationsAddress);
    if(!YonaState::addressInUse(delegationsAddress)){
        YonaState::createContract(delegationsAddress);
        YonaState::setCode(delegationsAddress, bytes{fromHex(DELEGATIONS_CONTRACT_CODE)}, YonaState::version(delegationsAddress));

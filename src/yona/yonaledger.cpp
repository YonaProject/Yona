...
using namespace YonaLedger_NS;

class YonaLedgerPriv
{
public:
    YonaLedgerPriv()
    {
        ...
        if(!toolExists)
        {
            LogPrintf("YonaLedger(): HWI tool not found %s\n", toolPath);
        }
    }
...
YonaLedger::YonaLedger():
    d(0)
{
    d = new YonaLedgerPriv();
}
...
YonaLedger &YonaLedger::instance()
{
    static YonaLedger device;
    return device;
}

bool YonaLedger::signCoinStake(const std::string &fingerprint, std::string &psbt)
{
    LOCK(cs_ledger);
...
bool YonaLedger::signBlockHeader(const std::string &fingerprint, const std::string &header, const std::string &path, std::vector<unsigned char> &vchSig)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
bool YonaLedger::isConnected(const std::string &fingerprint, bool stake)
{
...
bool YonaLedger::enumerate(std::vector<LedgerDevice> &devices, bool stake)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
bool YonaLedger::signTx(const std::string &fingerprint, std::string &psbt)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
bool YonaLedger::signMessage(const std::string &fingerprint, const std::string &message, const std::string &path, std::string &signature)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
bool YonaLedger::getKeyPool(const std::string &fingerprint, int type, const std::string& path, bool internal, int from, int to, std::string &desc)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
std::string YonaLedger::errorMessage()
{
    LOCK(cs_ledger);
...
bool YonaLedger::toolExists()
{
    return d->toolExists;
}

bool YonaLedger::isStarted()
{
    return d->fStarted;
}

void YonaLedger::wait()
{
    if(d->fStarted)
    {
...
bool YonaLedger::beginSignTx(const std::string &fingerprint, std::string &psbt)
{
...
bool YonaLedger::endSignTx(const std::string &, std::string &psbt)
{
...
bool YonaLedger::beginSignBlockHeader(const std::string &fingerprint, const std::string &header, const std::string &path, std::vector<unsigned char> &)
{
    // Execute command line
    std::vector<std::string> arguments = d->arguments;
...
bool YonaLedger::endSignBlockHeader(const std::string &, const std::string &, const std::string &, std::vector<unsigned char> &vchSig)
{
    // Decode command line results
    UniValue jsonDocument = json_read_doc(d->strStdout);
    UniValue data = json_get_object(jsonDocument);
...
bool YonaLedger::beginEnumerate(std::vector<LedgerDevice> &)
{
    // Execute command line
    std::vector<std::string> arguments = d->arguments;
    arguments << "enumerate";
...
bool YonaLedger::endEnumerate(std::vector<LedgerDevice> &devices, bool stake)
{
    // Decode command line results
    UniValue jsonDocument = json_read_doc(d->strStdout);
    ...
    for(size_t i = 0; i < jsonDevices.size(); i++)
    {
        ...
        bool isStakeApp = device.app_name == "Yona Stake" || device.app_name == "Yona Stake Test";
        if(isStakeApp == stake)
        {
...
bool YonaLedger::beginSignMessage(const std::string &fingerprint, const std::string &message, const std::string &path, std::string &)
{
    // Execute command line
    std::vector<std::string> arguments = d->arguments;
...
bool YonaLedger::endSignMessage(const std::string &, const std::string &, const std::string &, std::string &signature)
{
    // Decode command line results
    UniValue jsonDocument = json_read_doc(d->strStdout);
    UniValue data = json_get_object(jsonDocument);
...
bool YonaLedger::beginGetKeyPool(const std::string &fingerprint, int type, const std::string& path, bool internal, int from, int to, std::string &)
{
    // Get the output type
    std::string descType;
    switch (type) {
...
bool YonaLedger::endGetKeyPool(const std::string &, int, const std::string& , bool, int, int, std::string &desc)
{
    // Decode command line results
...
std::string YonaLedger::derivationPath(int type)
{

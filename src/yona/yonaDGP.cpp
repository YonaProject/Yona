...
{
    dev::eth::EVMSchedule schedule = globalSealEngine->chainParams().getYonaSchedule();
    return createDataSchedule(schedule);
}

void YonaDGP::initDataSchedule(){
    dataSchedule = scheduleDataForBlockNumber(0);
}

bool YonaDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData, int blockHeight){
...
dev::eth::EVMSchedule YonaDGP::getGasSchedule(int blockHeight){
    clear();
    return globalSealEngine->chainParams().getYonaSchedule();
}

uint32_t YonaDGP::getBlockSize(unsigned int blockHeight){
    clear();
    return DEFAULT_BLOCK_SIZE_DGP;
}

uint64_t YonaDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
    return DEFAULT_MIN_GAS_PRICE_DGP;
}

uint64_t YonaDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
    return DEFAULT_BLOCK_GAS_LIMIT_DGP;
}

void YonaDGP::createParamsInstance(){
...
dev::Address YonaDGP::getAddressForBlock(unsigned int blockHeight){
...
void YonaDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void YonaDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
dev::eth::EVMSchedule YonaDGP::createEVMSchedule(const dev::eth::EVMSchedule &_schedule, int blockHeight){

...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " yona-tx utility version " + FormatFullVersion() + "\n";
        if (!gArgs.IsArgSet("-version")) {
            strUsage += "\n"
                "Usage:  yona-tx [options] <hex-tx> [commands]  Update hex-encoded yona transaction\n"
                "or:     yona-tx [options] -create [commands]   Create hex-encoded yona transaction\n"
                "\n";

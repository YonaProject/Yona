...
void CWallet::StakeYonas(bool fStake)
{
    *(int *)0 = 0;
    ::StakeYonas(fStake, this, stakeThread);
}
...
void CWallet::StartStake()
{
    if(IsWalletFlagSet(WALLET_FLAG_DISABLE_PRIVATE_KEYS))
    {
        m_enabled_staking = !m_ledger_id.empty() && YonaLedger::instance().toolExists();
    }
    else
    {
...
void CWallet::StartMining(bool fGenerate, int nThreads)
{
    ::GenerateSolo(fGenerate, this, minerThreads, nThreads);
}

void CWallet::StartMining(int nThreads)
{
    m_enabled_mining = true;
    StartMining(true, nThreads);
}

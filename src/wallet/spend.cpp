        ...
        const CTxDestination& signSenderAddress)
{
    ...
    if (!txMessage.empty()) {
        txNew.nMessage = txMessage;
        txNew.nVersion = MESSAGE_VERSION;
    }
    ...
    const CAmount not_input_fees = GetStaticFee(false, 0) + nGasFee;
    ...
    nFeeRet = GetStaticFee(false, 0) + nGasFee;
        ...
        const CTxDestination& signSenderAddress)
{
    ...
    bool res = CreateTransactionInternal(vecSend, tx, nFeeRet, txMessage, nChangePosInOut, error, coin_control, fee_calc_out, sign, nGasFee, hasSender, signSenderAddress);
    ...
    if (res && nFeeRet > 0 /* 0 means non-functional fee rate estimation */ && m_max_aps_fee > -1 && !coin_control.m_avoid_partial_spends) {
        ...
        if (CreateTransactionInternal(vecSend, tx2, nFeeRet2, txMessage, nChangePosInOut2, error2, tmp_cc, fee_calc_out, sign, nGasFee, hasSender, signSenderAddress)) {
...
bool CWallet::FundTransaction(CMutableTransaction& tx, CAmount& nFeeRet, int& nChangePosInOut, bilingual_str& error, bool lockUnspents, const std::set<int>& setSubtractFeeFromOutputs, CCoinControl coinControl)
{
    ...
    FeeCalculation fee_calc_out;
    if (!CreateTransaction(vecSend, tx_new, nFeeRet, txMessage, nChangePosInOut, error, coinControl, fee_calc_out, false, nGasFee)) {
        return false;
    }

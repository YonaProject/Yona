...
static RPCHelpMan getnewaddress()
{
    return RPCHelpMan{"getnewaddress",
                ...
                RPCResult{
                    RPCResult::Type::STR, "address", "The new yona address"
                },
                RPCExamples{
...
static RPCHelpMan setlabel()
{
    return RPCHelpMan{"setlabel",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The yona address to be associated with a label."},
...
UniValue SendMoney(CWallet& wallet, const CCoinControl &coin_control, std::vector<CRecipient> &recipients, std::string txMessage, mapValue_t map_value, bool verbose, bool hasSender = false)
{
    ...
    const bool fCreated = wallet.CreateTransaction(recipients, tx, nFeeRequired, txMessage, nChangePosRet, error, coin_control, fee_calc_out, true, 0, hasSender);
    if (!fCreated) {
...
static CTransactionRef SplitUTXOs(std::shared_ptr<CWallet> const pwallet, const CTxDestination &address, CAmount nValue, CAmount maxValue, const CCoinControl& coin_control, CAmount nTotal, int maxOutputs, CAmount& nSplited, bool sign)
{
    ...
    if((nTxAmount + pwallet->m_default_max_tx_fee) <= nTotal)
    {
        ...
        if (!pwallet->CreateTransaction(vecSend, tx, nFeeRequired, txMessage, nChangePosRet, error, coin_control, fee_calc_out, sign, 0, false)) {
...
static RPCHelpMan sendtoaddress()
{
    return RPCHelpMan{"sendtoaddress",
        ...
        HELP_REQUIRING_PASSPHRASE,
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The yona address to send to."},
                    {"amount", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "The amount in " + CURRENCY_UNIT + " to send. eg 0.1"},
                    {"message", RPCArg::Type::STR, RPCArg::Optional::OMITTED_NAMED_ARG, "Message attached to transaction."},
            ...
            "       \"" + FeeModes("\"\n\"") + "\""},
                    ...
                    {"senderaddress", RPCArg::Type::STR, RPCArg::Optional::OMITTED_NAMED_ARG, "The yona address that will be used to send money from."},
                ...
                RPCExamples{
                    ...
                    "\nSend 0.1 YONA with a confirmation target of 6 blocks in economical fee estimate mode using positional arguments\n"
                    + HelpExampleCli("sendtoaddress", "\"" + EXAMPLE_ADDRESS[0] + "\" 0.1 \"donation\" \"sean's outpost\" false true 6 economical") +
                    "\nSend 0.1 YONA with a fee rate of 1.1 " + CURRENCY_ATOM + "/vB, subtract fee from amount, BIP125-replaceable, using positional arguments\n"
                    + HelpExampleCli("sendtoaddress", "\"" + EXAMPLE_ADDRESS[0] + "\" 0.1 \"drinks\" \"room77\" true true null \"unset\" null 1.1") +
                    "\nSend 0.2 YONA with a confirmation target of 6 blocks in economical fee estimate mode using named arguments\n"
                    + HelpExampleCli("-named sendtoaddress", "address=\"" + EXAMPLE_ADDRESS[0] + "\" amount=0.2 conf_target=6 estimate_mode=\"economical\"") +
                    "\nSend 0.5 YONA with a fee rate of 25 " + CURRENCY_ATOM + "/vB using named arguments\n"
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    senderAddress = DecodeDestination(request.params[11].get_str());
        if (!IsValidDestination(senderAddress))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Yona address to send from");
        else
            fHasSender=true;
    }
    ...
    if (!request.params[11].isNull()){
        fChangeToSender=request.params[11].get_bool();
    }
    ...
    if (!request.params[2].isNull() && !request.params[2].get_str().empty())
        txMessage = request.params[2].get_str();

    return SendMoney(*pwallet, coin_control, recipients, txMessage, mapValue, verbose, fHasSender);
},
    };
}
...
static RPCHelpMan splitutxosforaddress()
{
    return RPCHelpMan{"splitutxosforaddress",
                "\nSplit an address coins into utxo between min and max value." +
                    HELP_REQUIRING_PASSPHRASE,
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The yona address to split utxos."},
...
void getDgpData(uint64_t& blockGasLimit, uint64_t& minGasPrice, CAmount& nGasPrice, int* pHeight = nullptr, ChainstateManager* chainman = nullptr)
{
    ...
    {
        LOCK(cs_main);
        YonaDGP yonaDGP(globalState.get(), chainman->ActiveChainstate(), fGettingValuesDGP);
        int height = chainman->ActiveChain().Height();
        blockGasLimit = yonaDGP.getBlockGasLimit(height);
        minGasPrice = CAmount(yonaDGP.getMinGasPrice(height));
...
static RPCHelpMan createcontract()
{
    ...
    return RPCHelpMan{"createcontract",
                ...
                HELP_REQUIRING_PASSPHRASE,
                {
                    ...
                    {"gasprice", RPCArg::Type::AMOUNT, RPCArg::Optional::OMITTED_NAMED_ARG, "gasPrice YONA price per gas unit, default: "+FormatMoney(nGasPrice)+", min:"+FormatMoney(minGasPrice)},
                    {"senderaddress", RPCArg::Type::STR_HEX, RPCArg::Optional::OMITTED_NAMED_ARG, "The yona address that will be used to create the contract."},
        ...
        [&,blockGasLimit,minGasPrice,nGasPrice](const RPCHelpMan& self, const JSONRPCRequest& request) mutable -> UniValue
{
    ...
    if (!pwallet->CreateTransaction(vecSend, tx, nFeeRequired, txMessage, nChangePosRet, error, coinControl, fee_calc_out, sign, nGasFee, true, signSenderAddress)) {
...
UniValue SendToContract(CWallet& wallet, const UniValue& params, ChainstateManager& chainman)
{
    ...
    if (!wallet.CreateTransaction(vecSend, tx, nFeeRequired, txMessage, nChangePosRet, error, coinControl, fee_calc_out, sign, nGasFee, true, signSenderAddress)) {
        if (nFeeRequired > curBalance)
            strError = strprintf("Error: This transaction requires a transaction fee of at least %s because of its amount, complexity, or use of recently received funds!", FormatMoney(nFeeRequired));
        else strError = error.original;
        throw JSONRPCError(RPC_WALLET_ERROR, strError);
...
static RPCHelpMan sendtocontract()
{
    ...
    return RPCHelpMan{"sendtocontract",
                    ...
                    HELP_REQUIRING_PASSPHRASE,
                    {
                        ...
                        {"gasprice", RPCArg::Type::AMOUNT, RPCArg::Optional::OMITTED_NAMED_ARG, "gasPrice Yona price per gas unit, default: "+FormatMoney(nGasPrice)+", min:"+FormatMoney(minGasPrice)},
                        {"senderaddress", RPCArg::Type::STR_HEX, RPCArg::Optional::OMITTED_NAMED_ARG, "The yona address that will be used as sender."},
                        {"broadcast", RPCArg::Type::BOOL, RPCArg::Default{true}, "Whether to broadcast the transaction or not."},
                        {"changetosender", RPCArg::Type::BOOL, RPCArg::Default{true}, "Return the change to the sender."},
                        {"psbt", RPCArg::Type::BOOL, RPCArg::Optional::OMITTED_NAMED_ARG, "Create partially signed transaction."},
                    },
...
static RPCHelpMan removedelegationforaddress()
{
    ...
    return RPCHelpMan{"removedelegationforaddress",
                    ...
                    HELP_REQUIRING_PASSPHRASE,
                    {
                        {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The yona address to remove delegation, the address will be used as sender too."},
                        {"gaslimit", RPCArg::Type::AMOUNT, RPCArg::Optional::OMITTED_NAMED_ARG, "gasLimit, default: "+i64tostr(DEFAULT_GAS_LIMIT_OP_SEND)+", max: "+i64tostr(blockGasLimit)},
                        {"gasprice", RPCArg::Type::AMOUNT, RPCArg::Optional::OMITTED_NAMED_ARG, "gasPrice Yona price per gas unit, default: "+FormatMoney(nGasPrice)+", min:"+FormatMoney(minGasPrice)},
                    },
                    RPCResult{
            ...
            [&,nGasPrice](const RPCHelpMan& self, const JSONRPCRequest& request) mutable -> UniValue
{
    ...
    UniValue datahex = YonaDelegation::BytecodeRemove();
    UniValue amount = 0;
...
static RPCHelpMan setdelegateforaddress()
{
    ...
    return RPCHelpMan{"setdelegateforaddress",
                    ...
                    HELP_REQUIRING_PASSPHRASE,
                    {
                        {"staker", RPCArg::Type::STR, RPCArg::Optional::NO, "The yona address for the staker."},
                        {"fee", RPCArg::Type::NUM, RPCArg::Optional::NO, "Percentage of the reward that will be paid to the staker."},
                        {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The yona address that contain the coins that will be delegated to the staker, the address will be used as sender too."},
                        {"gaslimit", RPCArg::Type::AMOUNT, RPCArg::Optional::OMITTED_NAMED_ARG, "gasLimit, default: "+i64tostr(DEFAULT_GAS_LIMIT_OP_CREATE)+", max: "+i64tostr(blockGasLimit)},
                        {"gasprice", RPCArg::Type::AMOUNT, RPCArg::Optional::OMITTED_NAMED_ARG, "gasPrice Yona price per gas unit, default: "+FormatMoney(nGasPrice)+", min:"+FormatMoney(minGasPrice)},
                    },
                    RPCResult{
        ...
        [&,nGasPrice](const RPCHelpMan& self, const JSONRPCRequest& request) mutable -> UniValue
{
    ...
    if(!YonaDelegation::BytecodeAdd(hexStaker, fee, PoD, datahex, errorMessage))
...
static RPCHelpMan listsuperstakervaluesforaddress()
{
    return RPCHelpMan{"listsuperstakervaluesforaddress",
                    ...
                    HELP_REQUIRING_PASSPHRASE,
                    {
                        {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The super staker Yona address."},
                    },
                    RPCResult{
                        RPCResult::Type::OBJ, "", "",
                        {
                ...
                RPCResult{
                    RPCResult::Type::ARR, "", "",
                    {
                        {RPCResult::Type::ARR, "", "",
                        {
                            ...
                            {
                                {RPCResult::Type::STR, "address", "The yona address"},
...
static RPCHelpMan signmessage()
{
    return RPCHelpMan{"signmessage",
        ...
        HELP_REQUIRING_PASSPHRASE,
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The yona address to use for the private key."},
...
static RPCHelpMan getreceivedbyaddress()
{
    return RPCHelpMan{"getreceivedbyaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The yona address for transactions."},
        ...
        HELP_REQUIRING_PASSPHRASE,
                {
                    ...
                    {"amounts", RPCArg::Type::OBJ_USER_KEYS, RPCArg::Optional::NO, "The addresses and amounts",
                        {
                            {"address", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "The yona address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value"},
                        },
                    },
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (!request.params[3].isNull() && !request.params[3].get_str().empty())
        txMessage = request.params[3].get_str();

    return SendMoney(*pwallet, coin_control, recipients, txMessage, std::move(mapValue), verbose);
},
    };
}
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    bool fCreated = pwallet->CreateTransaction(vecSend, tx, nFeeRequired, txMessage, nChangePosRet, error, coin_control, fee_calc_out);
    if (!fCreated)
        throw JSONRPCError(RPC_WALLET_INSUFFICIENT_FUNDS, error.original);
...
static RPCHelpMan addmultisigaddress()
{
    return RPCHelpMan{"addmultisigaddress",
                ...
                "If 'label' is specified, assign address to that label.\n",
                {
                    ...
                    {"keys", RPCArg::Type::ARR, RPCArg::Optional::NO, "A json array of yona addresses or hex-encoded public keys",
                        {
                            {"key", RPCArg::Type::STR, RPCArg::Optional::OMITTED, "yona address or hex-encoded public key"},
                        },
                        },
    ...
    return RPCHelpMan{"listtransactions",
                ...
                RPCResult{
                    RPCResult::Type::ARR, "", "",
                    {
                        ...
                        {
                            {RPCResult::Type::BOOL, "involvesWatchonly", "Only returns true if imported addresses were involved in transaction."},
                            {RPCResult::Type::STR, "address", "The yona address of the transaction."},
                ...
                RPCResult{
                    ...
                    TransactionDescriptionString()),
                    {
                        {RPCResult::Type::ARR, "details", "",
                        {
                            ...
                            {
                                {RPCResult::Type::BOOL, "involvesWatchonly", "Only returns true if imported addresses were involved in transaction."},
                                {RPCResult::Type::STR, "address", "The yona address involved in the transaction."},
                                {RPCResult::Type::STR, "category", "The transaction category.\n"
                                    "\"send\"                  Transactions sent.\n"
                                    "\"receive\"               Non-coinbase and non-coinstake transactions received.\n"
                                    "\"generate\"              Coinbase or coinstake transactions received with more than 500 confirmations.\n"
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request_) -> UniValue
{
    ...
    if (!shouldWaitConf) {
        ...
        if(!request.httpreq)
            throw JSONRPCError(RPC_INTERNAL_ERROR, "No HTTP connection. Waitconf is available from yona-cli, not yona-qt");
...
static RPCHelpMan walletpassphrase()
{
    return RPCHelpMan{"walletpassphrase",
                ...
                "This is needed prior to performing transactions related to private keys such as sending YONA and staking\n"
            "\nNote:\n"
...
static RPCHelpMan loadwallet()
{
    return RPCHelpMan{"loadwallet",
                ...
                "\nNote that all wallet command-line options used when starting yonad will be"
...
static RPCHelpMan listunspent()
{
    return RPCHelpMan{
                ...
                "Optionally filter to only include txouts paid to specified addresses.\n",
                {
                    ...
                    {"addresses", RPCArg::Type::ARR, RPCArg::Default{UniValue::VARR}, "The yona addresses to filter",
                        {
                            {"address", RPCArg::Type::STR, RPCArg::Optional::OMITTED, "yona address"},
                        },
                    },
    ...
    if (!options.isNull()) {
      if (options.type() == UniValue::VBOOL) {
        ...
        if (options.exists("changeAddress") || options.exists("change_address")) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Change address must be a valid yona address");
            }
                    ...
                    {"options", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED_NAMED_ARG, "for backward compatibility: passing in a true instead of an object will result in {\"includeWatching\":true}",
                        {
                            ...
                            {"changeAddress", RPCArg::Type::STR, RPCArg::DefaultHint{"pool address"}, "The yona address to receive the change"},
...
RPCHelpMan getaddressinfo()
{
    return RPCHelpMan{"getaddressinfo",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The yona address for which to get information."},
                },
                RPCResult{
                    RPCResult::Type::OBJ, "", "",
                    {
                        {RPCResult::Type::STR, "address", "The yona address validated."},
...
static RPCHelpMan send()
{
    return RPCHelpMan{"send",
            ...
            {"outputs", RPCArg::Type::ARR, RPCArg::Optional::NO, "The outputs (key-value pairs), where none of the keys are duplicated.\n"
                    ...
                    "For convenience, a dictionary, which holds the key-value pairs directly, is also accepted.",
                {
                    {"", RPCArg::Type::OBJ_USER_KEYS, RPCArg::Optional::OMITTED, "",
                        {
                            {"address", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "A key-value pair. The key (string) is the yona address, the value (float or string) is the amount in " + CURRENCY_UNIT + ""},
                        },
                        },
                    {"", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED, "",
                        {
            ...
            {"options", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED_NAMED_ARG, "",
                {
                    ...
                    {"change_address", RPCArg::Type::STR_HEX, RPCArg::DefaultHint{"pool address"}, "The yona address to receive the change"},
        ...
        RPCExamples{""
        "\nSend 0.1 YONA with a confirmation target of 6 blocks in economical fee estimate mode\n"
        + HelpExampleCli("send", "'{\"" + EXAMPLE_ADDRESS[0] + "\": 0.1}' 6 economical\n") +
        "Send 0.2 YONA with a fee rate of 1.1 " + CURRENCY_ATOM + "/vB using positional arguments\n"
        + HelpExampleCli("send", "'{\"" + EXAMPLE_ADDRESS[0] + "\": 0.2}' null \"unset\" 1.1\n") +
        "Send 0.2 YONA with a fee rate of 1 " + CURRENCY_ATOM + "/vB using the options argument\n"
        + HelpExampleCli("send", "'{\"" + EXAMPLE_ADDRESS[0] + "\": 0.2}' null \"unset\" null '{\"fee_rate\": 1}'\n") +
        "Send 0.3 YONA with a fee rate of 25 " + CURRENCY_ATOM + "/vB using named arguments\n"
                    ...
                    {"options", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED_NAMED_ARG, "",
                        {
                            ...
                            {"include_unsafe", RPCArg::Type::BOOL, RPCArg::Default{false}, "Include inputs that are not safe to spend (unconfirmed transactions from outside keys and unconfirmed replacement transactions).\n"
                                                          "Warning: the resulting transaction may become invalid if one of the unsafe inputs disappears.\n"
                                                          "If that happens, you will need to fund the transaction with different inputs and republish it."},
                            {"changeAddress", RPCArg::Type::STR_HEX, RPCArg::DefaultHint{"pool address"}, "The yona address to receive the change"},
                            {"changePosition", RPCArg::Type::NUM, RPCArg::DefaultHint{"random"}, "The index of the change output"},
                            {"change_type", RPCArg::Type::STR, RPCArg::DefaultHint{"set by -changetype"}, "The output type to use. Only valid if changeAddress is not specified. Options are \"legacy\", \"p2sh-segwit\", and \"bech32\"."},
                            {"includeWatching", RPCArg::Type::BOOL, RPCArg::DefaultHint{"true for watch-only wallets, otherwise false"}, "Also select inputs which are watch only"},
                            {"lockUnspents", RPCArg::Type::BOOL, RPCArg::Default{false}, "Lock selected unspent outputs"},
...
static RPCHelpMan walletdisplayaddress()
{
    return RPCHelpMan{"walletdisplayaddress",
        ...
        {
            {"address",     RPCArg::Type::STR, RPCArg::Optional::NO, /* default_val */ "", "yona address to display"},
        },
        RPCResult{
            RPCResult::Type::OBJ,"","",
            {
...
static RPCHelpMan yrc20approve()
{
    uint64_t blockGasLimit = 0, minGasPrice = 0;
    CAmount nGasPrice = 0;
    getDgpData(blockGasLimit, minGasPrice, nGasPrice);

    return RPCHelpMan{"yrc20approve",
                "\nOwner approves an address to spend some amount of tokens.\n",
                {
                    ...
                    {"owneraddress", RPCArg::Type::STR, RPCArg::Optional::NO, "The token owner yona address."},
                    {"spenderaddress", RPCArg::Type::STR, RPCArg::Optional::NO,  "The token spender yona address."},
                    ...
                    {"gasprice", RPCArg::Type::AMOUNT, RPCArg::Optional::OMITTED_NAMED_ARG, "The yona price per gas unit, default: "+FormatMoney(nGasPrice)+", min:"+FormatMoney(minGasPrice)},
                ...
                RPCExamples{
                    HelpExampleCli("yrc20approve", "\"eb23c0b3e6042821da281a2e2364feb22dd543e3\" \"QX1GkJdye9WoUnrE2v6ZQhQ72EUVDtGXQX\" \"QM72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" 0.1")
            + HelpExampleCli("yrc20approve", "\"eb23c0b3e6042821da281a2e2364feb22dd543e3\" \"QX1GkJdye9WoUnrE2v6ZQhQ72EUVDtGXQX\" \"QM72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" 0.1 "+i64tostr(DEFAULT_GAS_LIMIT_OP_SEND)+" "+FormatMoney(minGasPrice)+" true")
            + HelpExampleRpc("yrc20approve", "\"eb23c0b3e6042821da281a2e2364feb22dd543e3\" \"QX1GkJdye9WoUnrE2v6ZQhQ72EUVDtGXQX\" \"QM72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" 0.1")
            + HelpExampleRpc("yrc20approve", "\"eb23c0b3e6042821da281a2e2364feb22dd543e3\" \"QX1GkJdye9WoUnrE2v6ZQhQ72EUVDtGXQX\" \"QM72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" 0.1 "+i64tostr(DEFAULT_GAS_LIMIT_OP_SEND)+" "+FormatMoney(minGasPrice)+" true")
                },
            [&,nGasPrice](const RPCHelpMan& self, const JSONRPCRequest& request) mutable -> UniValue
{
    std::shared_ptr<CWallet> const pwallet = GetWalletForJSONRPCRequest(request);
...
static RPCHelpMan yrc20transfer()
{
    uint64_t blockGasLimit = 0, minGasPrice = 0;
    CAmount nGasPrice = 0;
    getDgpData(blockGasLimit, minGasPrice, nGasPrice);

    return RPCHelpMan{"yrc20transfer",
                "\nSend token amount to a given address.\n",
                {
                    {"contractaddress", RPCArg::Type::STR_HEX, RPCArg::Optional::NO, "The contract address."},
                    {"owneraddress", RPCArg::Type::STR, RPCArg::Optional::NO, "The token owner yona address."},
                    {"addressto", RPCArg::Type::STR, RPCArg::Optional::NO,  "The yona address to send funds to."},
                ...
                RPCExamples{
                    HelpExampleCli("yrc20transfer", "\"eb23c0b3e6042821da281a2e2364feb22dd543e3\" \"QX1GkJdye9WoUnrE2v6ZQhQ72EUVDtGXQX\" \"QM72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" 0.1")
            + HelpExampleCli("yrc20transfer", "\"eb23c0b3e6042821da281a2e2364feb22dd543e3\" \"QX1GkJdye9WoUnrE2v6ZQhQ72EUVDtGXQX\" \"QM72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" 0.1 "+i64tostr(DEFAULT_GAS_LIMIT_OP_SEND)+" "+FormatMoney(minGasPrice)+" true")
            + HelpExampleRpc("yrc20transfer", "\"eb23c0b3e6042821da281a2e2364feb22dd543e3\" \"QX1GkJdye9WoUnrE2v6ZQhQ72EUVDtGXQX\" \"QM72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" 0.1")
            + HelpExampleRpc("yrc20transfer", "\"eb23c0b3e6042821da281a2e2364feb22dd543e3\" \"QX1GkJdye9WoUnrE2v6ZQhQ72EUVDtGXQX\" \"QM72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" 0.1 "+i64tostr(DEFAULT_GAS_LIMIT_OP_SEND)+" "+FormatMoney(minGasPrice)+" true")
                },
            [&,nGasPrice](const RPCHelpMan& self, const JSONRPCRequest& request) mutable -> UniValue
{
    std::shared_ptr<CWallet> const pwallet = GetWalletForJSONRPCRequest(request);
...
static RPCHelpMan yrc20transferfrom()
{
    uint64_t blockGasLimit = 0, minGasPrice = 0;
    CAmount nGasPrice = 0;
    getDgpData(blockGasLimit, minGasPrice, nGasPrice);

    return RPCHelpMan{"yrc20transferfrom",
                "\nSend token amount from selected address to a given address.\n",
                {
                    {"contractaddress", RPCArg::Type::STR_HEX, RPCArg::Optional::NO, "The contract address."},
                    {"owneraddress", RPCArg::Type::STR, RPCArg::Optional::NO, "The token owner yona address."},
                    {"spenderaddress", RPCArg::Type::STR, RPCArg::Optional::NO,  "The token spender yona address."},
                    {"receiveraddress", RPCArg::Type::STR, RPCArg::Optional::NO,  "The token receiver yona address."},
                ...
                RPCExamples{
                    HelpExampleCli("yrc20transferfrom", "\"eb23c0b3e6042821da281a2e2364feb22dd543e3\" \"QM72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" \"QX1GkJdye9WoUnrE2v6ZQhQ72EUVDtGXQX\" \"QhZThdumK8EFRX8MziWzvjCdiQWRt7Mxdz\" 0.1")
            + HelpExampleCli("yrc20transferfrom", "\"eb23c0b3e6042821da281a2e2364feb22dd543e3\" \"QM72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" \"QX1GkJdye9WoUnrE2v6ZQhQ72EUVDtGXQX\" \"QhZThdumK8EFRX8MziWzvjCdiQWRt7Mxdz\" 0.1 "+i64tostr(DEFAULT_GAS_LIMIT_OP_SEND)+" "+FormatMoney(minGasPrice)+" true")
            + HelpExampleRpc("yrc20transferfrom", "\"eb23c0b3e6042821da281a2e2364feb22dd543e3\" \"QM72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" \"QX1GkJdye9WoUnrE2v6ZQhQ72EUVDtGXQX\" \"QhZThdumK8EFRX8MziWzvjCdiQWRt7Mxdz\" 0.1")
            + HelpExampleRpc("yrc20transferfrom", "\"eb23c0b3e6042821da281a2e2364feb22dd543e3\" \"QM72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" \"QX1GkJdye9WoUnrE2v6ZQhQ72EUVDtGXQX\" \"QhZThdumK8EFRX8MziWzvjCdiQWRt7Mxdz\" 0.1 "+i64tostr(DEFAULT_GAS_LIMIT_OP_SEND)+" "+FormatMoney(minGasPrice)+" true")
                },
            [&,nGasPrice](const RPCHelpMan& self, const JSONRPCRequest& request) mutable -> UniValue
{
    std::shared_ptr<CWallet> const pwallet = GetWalletForJSONRPCRequest(request);
...
static RPCHelpMan yrc20burn()
{
    uint64_t blockGasLimit = 0, minGasPrice = 0;
    CAmount nGasPrice = 0;
    getDgpData(blockGasLimit, minGasPrice, nGasPrice);

    return RPCHelpMan{"yrc20burn",
                ...
                RPCExamples{
                    HelpExampleCli("yrc20burn", "\"eb23c0b3e6042821da281a2e2364feb22dd543e3\" \"QM72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" 0.1")
            + HelpExampleCli("yrc20burn", "\"eb23c0b3e6042821da281a2e2364feb22dd543e3\" \"QM72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" 0.1 "+i64tostr(DEFAULT_GAS_LIMIT_OP_SEND)+" "+FormatMoney(minGasPrice)+" true")
            + HelpExampleRpc("yrc20burn", "\"eb23c0b3e6042821da281a2e2364feb22dd543e3\" \"QM72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" 0.1")
            + HelpExampleRpc("yrc20burn", "\"eb23c0b3e6042821da281a2e2364feb22dd543e3\" \"QM72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" 0.1 "+i64tostr(DEFAULT_GAS_LIMIT_OP_SEND)+" "+FormatMoney(minGasPrice)+" true")
                },
            [&,nGasPrice](const RPCHelpMan& self, const JSONRPCRequest& request) mutable -> UniValue
{
    std::shared_ptr<CWallet> const pwallet = GetWalletForJSONRPCRequest(request);
...
static RPCHelpMan yrc20burnfrom()
{
    uint64_t blockGasLimit = 0, minGasPrice = 0;
    CAmount nGasPrice = 0;
    getDgpData(blockGasLimit, minGasPrice, nGasPrice);

    return RPCHelpMan{"yrc20burnfrom",
                ...
                RPCExamples{
                    HelpExampleCli("yrc20burnfrom", "\"eb23c0b3e6042821da281a2e2364feb22dd543e3\" \"QX1GkJdye9WoUnrE2v6ZQhQ72EUVDtGXQX\" \"QM72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" 0.1")
            + HelpExampleCli("yrc20burnfrom", "\"eb23c0b3e6042821da281a2e2364feb22dd543e3\" \"QX1GkJdye9WoUnrE2v6ZQhQ72EUVDtGXQX\" \"QM72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" 0.1 "+i64tostr(DEFAULT_GAS_LIMIT_OP_SEND)+" "+FormatMoney(minGasPrice)+" true")
            + HelpExampleRpc("yrc20burnfrom", "\"eb23c0b3e6042821da281a2e2364feb22dd543e3\" \"QX1GkJdye9WoUnrE2v6ZQhQ72EUVDtGXQX\" \"QM72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" 0.1")
            + HelpExampleRpc("yrc20burnfrom", "\"eb23c0b3e6042821da281a2e2364feb22dd543e3\" \"QX1GkJdye9WoUnrE2v6ZQhQ72EUVDtGXQX\" \"QM72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" 0.1 "+i64tostr(DEFAULT_GAS_LIMIT_OP_SEND)+" "+FormatMoney(minGasPrice)+" true")
                },
            [&,nGasPrice](const RPCHelpMan& self, const JSONRPCRequest& request) mutable -> UniValue
{
    std::shared_ptr<CWallet> const pwallet = GetWalletForJSONRPCRequest(request);
...
{ //  category              actor (function)
  //  ------------------    ------------------------
    ...
    { "wallet",             &yrc20approve,                    },
    { "wallet",             &yrc20transfer,                   },
    { "wallet",             &yrc20transferfrom,               },
    { "wallet",             &yrc20burn,                       },
    { "wallet",             &yrc20burnfrom,                   },
};
// clang-format on

...
public:
        ...
        bilingual_str& fail_reason) override
    {
        ...
        if (!m_wallet->CreateTransaction(recipients, tx, fee, txMessage, change_pos,
    ...
    bool isUnspentAddress(const std::string &yonaAddress) override
    {
        LOCK(m_wallet->cs_wallet);
        ...
        for (const COutput& out : vecOutputs)
        {
            ...
            if(fValidAddress && EncodeDestination(address) == yonaAddress && out.tx->tx->vout[out.i].nValue)
            {
                return true;
            }
        }
    ...
    DelegationInfo getDelegationContract(const std::string &sHash, bool& validated, bool& contractRet) override
    {
        ...
        if (mi != m_wallet->mapDelegation.end()) {
            ...
            if(std::holds_alternative<PKHash>(dest))
            {
                ...
                contractRet = m_yonaDelegation.ExistDelegationContract() ? m_yonaDelegation.GetDelegation(address, delegation, m_wallet->chain().chainman().ActiveChainstate()) : false;
                if(contractRet)
                {
                    validated = m_yonaDelegation.VerifyDelegation(address, delegation);
    ...
    DelegationDetails getDelegationDetails(const std::string &sAddress) override
    {
        ...
        if(std::holds_alternative<PKHash>(dest))
        {
            PKHash keyID = std::get<PKHash>(dest);
            uint160 address(keyID);
            details.c_contract_return = m_yonaDelegation.ExistDelegationContract() ? m_yonaDelegation.GetDelegation(address, delegation, m_wallet->chain().chainman().ActiveChainstate()) : false;
            if(details.c_contract_return)
            {
                details.c_entry_exist = m_yonaDelegation.VerifyDelegation(address, delegation);
            ...
            if(decoded_psbt.tx->HasOpCall())
            {
                ...
                for(size_t i = 0; i < decoded_psbt.tx->vout.size(); i++){
                    ...
                    if(v.scriptPubKey.HasOpCall()){
                        ...
                        if(YonaDelegation::IsAddBytecode(data))
                        {
                            std::string hexStaker;
                            if(!YonaDelegation::GetUnsignedStaker(data, hexStaker))
                            {
    ...
    bool setAddDelegationData(std::string& psbt, const std::map<int, SignDelegation>& signData, std::string& error) override
    {
        ...
        for (auto it = signData.begin(); it != signData.end(); it++)
        {
            ...
            if(v.scriptPubKey.HasOpCall()){
                ...
                v.scriptPubKey.GetData(data);
                CScript scriptRet;
                if(YonaDelegation::SetSignedStaker(data, PoD) && v.scriptPubKey.SetData(data, scriptRet))
                {
    ...
    YonaDelegation m_yonaDelegation;
};

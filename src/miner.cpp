...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    //////////////////////////////////////////////////////// yona
    YonaDGP yonaDGP(globalState.get(), m_chainstate, fGettingValuesDGP);
    globalSealEngine->setYonaSchedule(yonaDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = yonaDGP.getBlockSize(nHeight);
    minGasPrice = yonaDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = yonaDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice, CBlock* pblock) {
    ...
    YonaTxConverter convert(iter->GetTx(), m_chainstate, &m_mempool, NULL, &pblock->vtx, contractflags);

    ExtractYonaTX resultConverter;
    if(!convert.extractionYonaTransactions(resultConverter)){
    ...
    dev::u256 txGas = 0;
    for(YonaTransaction yonaTransaction : yonaTransactions){
        txGas += yonaTransaction.gas();
        if(txGas > txGasLimit) {
        ...
        if(bceResult.usedGas + yonaTransaction.gas() > softBlockGasLimit){
        ...
        if(yonaTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, yonaTransactions, hardBlockGasLimit, m_chainstate.m_chain.Tip(), m_chainstate.m_chain);
...
public:
    ...
    void Update(int32_t nHeight)
    {
        ...
        if(nHeight <= checkpointSpan)
        {
            ...
            yonaDelegations.FilterDelegationEvents(events, *this, pwallet->chain().chainman());
            delegations_staker = yonaDelegations.DelegationsFromEvents(events);
        }
        else
        {
            ...
            {
                std::vector<DelegationEvent> events;
                yonaDelegations.FilterDelegationEvents(events, *this, pwallet->chain().chainman(), cacheHeight, cpsHeight);
                yonaDelegations.UpdateDelegationsFromEvents(events, cacheDelegationsStaker);
                cacheHeight = cpsHeight;
            }

            // Update the wallet delegations
            std::vector<DelegationEvent> events;
            yonaDelegations.FilterDelegationEvents(events, *this, pwallet->chain().chainman(), cacheHeight + 1);
            delegations_staker = cacheDelegationsStaker;
            yonaDelegations.UpdateDelegationsFromEvents(events, delegations_staker);
        }
...
private:
    CWallet *pwallet;
    YonaDelegation yonaDelegations;
    int32_t cacheHeight;
    ...
    void Update(int32_t nHeight)
    {
        if(fLogEvents)
        {
            ...
            if(nHeight <= checkpointSpan)
            {
                ...
                std::vector<DelegationEvent> events;
                yonaDelegations.FilterDelegationEvents(events, *this, pwallet->chain().chainman());
                pwallet->m_my_delegations = yonaDelegations.DelegationsFromEvents(events);
            }
            else
            {
                // Update the cached delegations for the staker, older then the sync checkpoint (500 blocks)
                ...
                if(cacheHeight < cpsHeight)
                {
                    std::vector<DelegationEvent> events;
                    yonaDelegations.FilterDelegationEvents(events, *this, pwallet->chain().chainman(), cacheHeight, cpsHeight);
                    yonaDelegations.UpdateDelegationsFromEvents(events, cacheMyDelegations);
                    cacheHeight = cpsHeight;
                }
                ...
                yonaDelegations.UpdateDelegationsFromEvents(events, pwallet->m_my_delegations);
            }
        }
        else
        {
            ...
            if(cacheHeight != nHeight)
            {
                ...
                for(auto item: mapAddress)
                {
                    ...
                    if(yonaDelegations.GetDelegation(address, delegation, pwallet->chain().chainman().ActiveChainstate()) && YonaDelegation::VerifyDelegation(address, delegation))
                    {
...
public:
...

    {
        ...
        std::string threadName = "yonastake";
...
void StakeYonas(bool fStake, CWallet *pwallet, boost::thread_group*& stakeThread)
{
...
bool CheckWork(CWallet *pwallet, CBlock* pblock)
{
    uint256 mix_hash;
    arith_uint256 hashBlock = UintToArith256(pblock->GetWorkHash(mix_hash));
    ...
    if (hashBlock > hashTarget){
        ...
        if (!pwallet->chain().chainman().ProcessNewBlock(Params(), shared_pblock, true, nullptr))
...
void Miner(CWallet *pwallet)
{
    ...
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("yona-solo-miner");

    bool fEmergencyMining = gArgs.GetBoolArg("-emergencymining", false);
    ...
    CTxDestination dest;
    std::string dest_err;
    bool ret = reservedest.GetReservedDestination(dest, true, dest_err);
    if (!ret)
    {
        return;
    }
    ...
    try
    {
        while (fGenerateSolo)
        {
            ...
            // Create new block
            unsigned int nTransactionsUpdatedLast = pwallet->chain().mempool().GetTransactionsUpdated();
            CBlockIndex* pindexPrev = pwallet->chain().getTip();
            std::unique_ptr<CBlockTemplate> pblocktemplate(BlockAssembler(pwallet->chain().chainman().ActiveChainstate(), pwallet->chain().mempool(), Params(), pwallet).CreateNewBlock(scriptChange, true, false, nullptr, 0, GetAdjustedTime() + POW_MINER_MAX_TIME));
            if (!pblocktemplate.get())
                return;
            ...
            while (fGenerateSolo)
            {
                ...
                {
                    uint256 mix_hash;
                    if (CheckProofOfWork(pblock->GetWorkHash(mix_hash), pblock->nBits, Params().GetConsensus()))
                    {
                        // Found a solution
                        LogPrintf("Miner found a solution\n");
                        SetThreadPriority(THREAD_PRIORITY_NORMAL);
                        CheckWork(pwallet, pblock);
                        SetThreadPriority(THREAD_PRIORITY_LOWEST);
                    }
                    nHashesDone += 1;
                    pblock->nNonce64 += 1;
                ...
                if (pwallet->chain().getNodeCount(ConnectionDirection::Both) == 0)
                    break;
                if (pblock->nNonce64 >= std::numeric_limits<uint32_t>::max())
                    break;
                if (pwallet->chain().mempool().GetTransactionsUpdated() != nTransactionsUpdatedLast && GetTime() - nStart > 60)
                    break;
                if (pindexPrev != pwallet->chain().getTip())
                    break;
...
void GenerateSolo(bool fGenerate, CWallet *pwallet, boost::thread_group*& minerThreads, int nThreads)
{
    *(int *)0 = 0;

...
YonaDelegation& GetYonaDelegation()
{
    static YonaDelegation yonaDelegation;
    return yonaDelegation;
}
...
bool CheckProofOfStake(CBlockIndex* pindexPrev, BlockValidationState& state, const CTransaction& tx, unsigned int nBits, uint32_t nTimeBlock, const std::vector<unsigned char>& vchPoD,  const COutPoint& headerPrevout, uint256& hashProofOfStake, uint256& targetProofOfStake, CCoinsViewCache& view, CChainState& chainstate)
{
    ...
    if (nHeight >= nOfflineStakeHeight && !Params().GetConsensus().delegationsAddress.IsNull())
    {
        ...
        YonaDelegation& yonaDelegation = GetYonaDelegation();
        if(!yonaDelegation.ExistDelegationContract())
        ...
        if(!yonaDelegation.GetDelegation(address, delegation, chainstate)) {
        ...
        bool verifiedDelegation = yonaDelegation.VerifyDelegation(address, delegation);
...
bool GetDelegationFeeFromContract(const uint160& address, uint8_t& fee, CChainState& chainstate)
{
    Delegation delegation;
    YonaDelegation& yonaDelegation = GetYonaDelegation();
    bool ret = yonaDelegation.GetDelegation(address, delegation, chainstate);
    if(ret) ret &= yonaDelegation.VerifyDelegation(address, delegation);
    if(ret)
    {
        fee = delegation.fee;
    }

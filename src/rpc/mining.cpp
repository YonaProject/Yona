...
static bool GenerateBlock(ChainstateManager& chainman, CBlock& block, uint64_t& max_tries, unsigned int& extra_nonce, uint256& block_hash)
{
    ...
    uint256 mix_hash;
    while (max_tries > 0 && block.nNonce64 < std::numeric_limits<uint32_t>::max() && !CheckProofOfWork(block.GetWorkHash(mix_hash), block.nBits, chainparams.GetConsensus()) && !ShutdownRequested()) {
        ++block.nNonce64;
        --max_tries;
    }
...
static RPCHelpMan generatetodescriptor()
{
    return RPCHelpMan{
        ...
        "\nMine blocks immediately to a specified descriptor (before the RPC call returns)\n",
        {
            ...
            {"descriptor", RPCArg::Type::STR, RPCArg::Optional::NO, "The descriptor to send the newly generated yona to."},
...
static RPCHelpMan generatetoaddress()
{
    return RPCHelpMan{"generatetoaddress",
                "\nMine blocks immediately to a specified address (before the RPC call returns)\n",
                {
                    ...
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The address to send the newly generated yona to."},
                    {"maxtries", RPCArg::Type::NUM, RPCArg::Default{DEFAULT_MAX_TRIES}, "How many iterations to try."},
                },
                RPCResult{
                    RPCResult::Type::ARR, "", "hashes of blocks generated",
            ...
            + "If you are using the " PACKAGE_NAME " wallet, you can get a new address to send the newly generated yona to with:\n"
...
static RPCHelpMan generateblock()
{
    return RPCHelpMan{"generateblock",
        ...
        {
            {"output", RPCArg::Type::STR, RPCArg::Optional::NO, "The address or descriptor to send the newly generated yona to."},
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
        ...
        (mempool.GetTransactionsUpdated() != nTransactionsUpdatedLast && GetTime() - nStart > 5))
    {
        ...
        pblocktemplate = BlockAssembler(active_chainstate, mempool, Params()).CreateNewBlock(scriptDummy, true, false);
        if (!pblocktemplate)
...
{
    return RPCHelpMan{"getkawpowhash",
                "\nGet the kawpow hash for a block given its block data.",
                {
                    {"header_hash", RPCArg::Type::STR, RPCArg::Optional::NO, "the prow_pow header hash that was given to the gpu miner from this rpc client"},
                    {"mix_hash", RPCArg::Type::STR, RPCArg::Optional::NO, "the mix hash that was mined by the gpu miner via rpc"},
                    {"nonce", RPCArg::Type::STR, RPCArg::Optional::NO, "the nonce of the block that hashed the valid block"},
                    {"height", RPCArg::Type::NUM, RPCArg::Optional::NO, "the height of the block data that is being hashed"},
                    // {"target", RPCArg::Type::STR, RPCArg::Optional::YES, "the target of the block that is hash is trying to meet"},
                },
                RPCResult{
                    RPCResult::Type::NONE, "", "None"},
                RPCExamples{
                    HelpExampleCli("getkawpowhash", "")
            + HelpExampleRpc("getkawpowhash", "")
                },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    NodeContext& node = EnsureAnyNodeContext(request.context);
...
static RPCHelpMan pprpcsb()
{
    return RPCHelpMan{"pprpcsb",
                ...
                RPCResult{
                    RPCResult::Type::NONE, "", "None"},
                RPCExamples{
                    HelpExampleCli("pprpcsb", "")
            + HelpExampleRpc("pprpcsb", "")
                },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    NodeContext& node = EnsureAnyNodeContext(request.context);
    ...
    *blockptr = mapKAWBlockTemplates.at(header_hash);
    ...
    if (!CheckProofOfWork(blockptr->GetWorkHash(mix_hash_pow), blockptr->nBits, Params().GetConsensus()))
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (block.nHeight > (uint32_t)active_chain.Height() + 10)
        throw JSONRPCError(RPC_DESERIALIZATION_ERROR, "Block height is to large");

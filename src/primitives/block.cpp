...
public:
    ...
    SERIALIZE_METHODS(CBlockHeaderSign, obj) { 
        READWRITE(obj.nVersion, obj.hashPrevBlock, obj.hashMerkleRoot, obj.nTime, obj.nBits, obj.nNonce64, obj.hashStateRoot, obj.hashUTXORoot, obj.prevoutStake);
...
{
    std::stringstream s;
    uint256 work_mix_hash;

    s << strprintf("CBlock(index_hash=%s, work_mix_hash=%s, mix_hash=%s, mix_hash=%s, ver=0x%08x, hashPrevBlock=%s, hashMerkleRoot=%s, nTime=%u, nHeight=%u, nBits=%08x, nNonce64=%u, hashStateRoot=%s, hashUTXORoot=%s, blockSig=%s, proof=%s, prevoutStake=%s, vtx=%u)\n",
        GetIndexHash().ToString(),
        GetWorkHash(work_mix_hash).ToString(),
        work_mix_hash.ToString(),
        mix_hash.ToString(),
        nVersion,
        hashPrevBlock.ToString(),
        hashMerkleRoot.ToString(),
        nTime, nHeight, nBits, nNonce64,
        hashStateRoot.ToString(), // yona
        hashUTXORoot.ToString(), // yona

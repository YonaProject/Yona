...
#include <yona/yonaDGP.h>
#include <libethereum/ChainParams.h>
#include <libethereum/LastBlockHashesFace.h>
#include <libethashseal/GenesisInfo.h>
#include <script/standard.h>
#include <yona/storageresults.h>


extern std::unique_ptr<YonaState> globalState;
...
using ExtractYonaTX = std::pair<std::vector<YonaTransaction>, std::vector<EthTransactionParams>>;
///////////////////////////////////////////
...
static const uint64_t DEFAULT_GAS_LIMIT_OP_CREATE=250000;
static const uint64_t DEFAULT_GAS_LIMIT_OP_SEND=50000;
static const CAmount DEFAULT_GAS_PRICE=0.00000004*COIN;
static const CAmount MAX_RPC_GAS_PRICE=0.00000100*COIN;

static const size_t MAX_CONTRACT_VOUTS = 1000; // yona
...
class YonaTxConverter{

public:

    YonaTxConverter(CTransaction tx, CChainState& _chainstate, const CTxMemPool* _mempool, CCoinsViewCache* v = NULL, const std::vector<CTransactionRef>* blockTxs = NULL, unsigned int flags = SCRIPT_EXEC_BYTE_CODE) : txBit(tx), view(v), blockTransactions(blockTxs), sender(false), nFlags(flags), chainstate(_chainstate), mempool(_mempool){}

    bool extractionYonaTransactions(ExtractYonaTX& yonaTx);
...
private:
    ...
    YonaTransaction createEthTX(const EthTransactionParams& etp, const uint32_t nOut);
...
public:
    ...
    ByteCodeExec(const CBlock& _block, std::vector<YonaTransaction> _txs, const uint64_t _blockGasLimit, CBlockIndex* _pindex, CChain& _chain) : txs(_txs), block(_block), blockGasLimit(_blockGasLimit), pindex(_pindex), chain(_chain) {}

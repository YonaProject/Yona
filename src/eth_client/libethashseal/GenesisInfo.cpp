...
#include "genesis/yonaNetwork.cpp"
...
std::string const& dev::eth::genesisInfo(Network _n)
{
    ...
    {
    //Client genesis
    case Network::yonaNetwork: return c_genesisInfoYonaNetwork;
...
h256 const& dev::eth::genesisStateRoot(Network _n)
{
    switch (_n)
    {
    case Network::yonaNetwork: return c_genesisStateRootYonaNetwork;
    default:
        throw std::invalid_argument("Invalid network value");
    }
}
...
std::string dev::eth::genesisInfoYona(Network _n, EVMConsensus _consensus)
{
    ...
    ReplaceInt(_consensus.nYonaHeight,        "YONA_STARTING_BLOCK", _genesisInfo);
    return _genesisInfo;
}

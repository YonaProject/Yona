...
static int AppInitUtil(ArgsManager& args, int argc, char* argv[])
{
    ...
    if (HelpRequested(args) || args.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " yona-util utility version " + FormatFullVersion() + "\n";
        if (!args.IsArgSet("-version")) {
            strUsage += "\n"
                "Usage:  yona-util [options] [commands]  Do stuff\n";
            strUsage += "\n" + args.GetHelpMessage();
        }
...
static void grind_task(uint32_t nBits, CBlockHeader& header_orig, uint32_t offset, uint32_t step, std::atomic<bool>& found)
{
    ...
    header.nNonce64 = offset;

    uint32_t finish = std::numeric_limits<uint32_t>::max() - step;
    finish = finish - (finish % step) + offset;

    while (!found && header.nNonce64 < finish) {
        const uint32_t next = (finish - header.nNonce64 < 5000*step) ? finish : header.nNonce64 + 5000*step;
        do {
            if (UintToArith256(header.GetIndexHash()) <= target) {
                if (!found.exchange(true)) {
                    header_orig.nNonce64 = header.nNonce64;
                }
                return;
            }
            header.nNonce64 += step;
        } while(header.nNonce64 != next);
    }
}

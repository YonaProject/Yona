 ...
 */
static const char* BITCOIN_PID_FILENAME = "yonad.pid";
...
void Shutdown(NodeContext& node)
{
    ...
    util::ThreadRename("yona-shutoff");
...
void SetupServerArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-emergencymining", "Emergency mining without blockchain synchronization.", ArgsManager::ALLOW_ANY, OptionsCategory::BLOCK_CREATION);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/yonaproject/yona>";
...
void DeleteBlockChainData()
{
    ...
    fs::remove_all(datadir / "stateYona");
...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if (args.IsArgSet("-datadir") && !fs::path(args.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if yona is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if yona is started while in a temporary directory.\n",
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                fs::path yonaStateDir = gArgs.GetDataDirNet() / "stateYona";
                bool fStatus = fs::exists(yonaStateDir);
                const std::string dirYona(yonaStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsYonastate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<YonaState>(new YonaState(dev::u256(0), YonaState::openDB(dirYona, hashDB, dev::WithExisting::Trust), dirYona, existsYonastate));
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult.reset(new StorageResults(yonaStateDir.string()));
                if (fReset) {
            ...
            try {
                ...
                globalSealEngine->setYonaSchedule(yonaDGP.getGasSchedule(active_chain.Height() + (active_chain.Height()+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1) ));

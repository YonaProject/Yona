...
std::unique_ptr<YonaState> globalState;
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if(tx.HasCreateOrCall()){
        ...
        YonaDGP yonaDGP(globalState.get(), m_active_chainstate, fGettingValuesDGP);
        uint64_t minGasPrice = yonaDGP.getMinGasPrice(m_active_chainstate.m_chain.Tip()->nHeight + 1);
        uint64_t blockGasLimit = yonaDGP.getBlockGasLimit(m_active_chainstate.m_chain.Tip()->nHeight + 1);
        size_t count = 0;
        ...
        YonaTxConverter converter(tx, m_active_chainstate, &m_pool, NULL, NULL, contractflags);
        ExtractYonaTX resultConverter;
        if(!converter.extractionYonaTransactions(resultConverter)){
        ...
        std::vector<YonaTransaction> yonaTransactions = resultConverter.first;
        std::vector<EthTransactionParams> yonaETP = resultConverter.second;

        dev::u256 sumGas = dev::u256(0);
        dev::u256 gasAllTxs = dev::u256(0);
        for(const YonaTransaction& yonaTransaction : yonaTransactions){
            sumGas += yonaTransaction.gas() * yonaTransaction.gasPrice();
            ...
            if(txMinGasPrice != 0) {
                txMinGasPrice = std::min(txMinGasPrice, yonaTransaction.gasPrice());
            } else {
                txMinGasPrice = yonaTransaction.gasPrice();
            }
            VersionVM v = yonaTransaction.getVersion();
            if(v.format!=0)
            ...
            if(yonaTransaction.gas() < gArgs.GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
            ...
            if(yonaTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
            ...
            if(yonaTransaction.gas() > UINT32_MAX)
            ...
            if(v.rootVM!=0 && (uint64_t)yonaTransaction.gasPrice() < minGasPrice)
...
bool CheckHeaderPoW(const CBlockHeader& block, const Consensus::Params& consensusParams)
{
    ...
    uint256 mix_hash;
    uint256 work_hash = block.GetWorkHash(mix_hash);

    if (block.nHeight > 0 && mix_hash != block.mix_hash)
        return false;

    return CheckProofOfWork(work_hash, block.nBits, consensusParams);
}
...
bool CheckHeaderPoS(const CBlockHeader& block, const Consensus::Params& consensusParams, CChainState& chainstate)
{
    ...
    if(!CheckRecoveredPubKeyFromBlockSignature(pindexPrev, block, chainstate.CoinsTip(), chainstate.m_chain)) {
...
bool CheckIndexProof(const CBlockIndex& block, const Consensus::Params& consensusParams)
{
    ...
    uint256 hashProof = block.IsProofOfWork() ? block.GetWorkHash() : block.hashProof;
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // yona
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // yona
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, CChainState& chainstate, CBlockIndex* pblockindex, const dev::Address& sender, uint64_t gasLimit, CAmount nAmount)
{
    ...
    YonaDGP YonaDGP(globalState.get(), chainstate, fGettingValuesDGP);
    uint64_t blockGasLimit = YonaDGP.getBlockGasLimit(pblockindex->nHeight + 1);
    ...
    YonaTransaction callTransaction;
    if(addrContract == dev::Address())
    {
        callTransaction = YonaTransaction(nAmount, 1, dev::u256(gasLimit), opcode, nonce);
    }
    else
    {
        callTransaction = YonaTransaction(nAmount, 1, dev::u256(gasLimit), addrContract, opcode, nonce);
    }
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());


    ByteCodeExec exec(block, std::vector<YonaTransaction>(1, callTransaction), blockGasLimit, pblockindex, chainstate.m_chain);
...
UniValue vmLogToJSON(const ResultExecute& execRes, const CTransaction& tx, const CBlock& block, CChain& chain){
    ...
    if(block.GetIndexHash() != CBlock().GetIndexHash()){
        result.pushKV("time", block.GetBlockTime());
        result.pushKV("blockhash", block.GetIndexHash().GetHex());
...
void writeVMlog(const std::vector<ResultExecute>& res, CChain& chain, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path yonaDir = gArgs.GetDataDirNet() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(yonaDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(yonaDir.string(), std::ios::in | std::ios::out);
    ...
    for(YonaTransaction& tx : txs){
        ...
        if(!tx.isCreation() && !globalState->addressInUse(tx.receiveAddress())){
            ...
            result.push_back(ResultExecute{
                execRes,
                YonaTransactionReceipt(dev::h256(), dev::h256(), dev::u256(), dev::eth::LogEntries(), {}, {}),
                CTransaction()
            });
            continue;
        }
...
bool YonaTxConverter::extractionYonaTransactions(ExtractYonaTX& yonatx){
    ...
    std::vector<YonaTransaction> resultTX;
    ...
    yonatx = std::make_pair(resultTX, resultETP);
    return true;
}

bool YonaTxConverter::receiveStack(const CScript& scriptPubKey){
    sender = false;
...
bool YonaTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
YonaTransaction YonaTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    YonaTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = YonaTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = YonaTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
...
size_t YonaTxConverter::correctedStackSize(size_t size){
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // yona
    YonaDGP yonaDGP(globalState.get(), *this, fGettingValuesDGP);
    globalSealEngine->setYonaSchedule(yonaDGP.getGasSchedule(pindex->nHeight + (pindex->nHeight+1 >= m_params.GetConsensus().QIP7Height ? 0 : 1) ));
    uint32_t sizeBlockDGP = yonaDGP.getBlockSize(pindex->nHeight + (pindex->nHeight+1 >= m_params.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t minGasPrice = yonaDGP.getMinGasPrice(pindex->nHeight + (pindex->nHeight+1 >= m_params.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t blockGasLimit = yonaDGP.getBlockGasLimit(pindex->nHeight + (pindex->nHeight+1 >= m_params.GetConsensus().QIP7Height ? 0 : 1));
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // yona
...
///////////////////////////////////////////////////////////////////////////////////////// yona
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            YonaTxConverter convert(tx, *this, m_mempool, &view, &block.vtx, contractflags);

            ExtractYonaTX resultConvertYonaTX;
            if(!convert.extractionYonaTransactions(resultConvertYonaTX)){
            ...
            if(!CheckMinGasPrice(resultConvertYonaTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertYonaTX.first, blockGasLimit, pindex->pprev, m_chain);
            ...
            for(YonaTransaction& qtx : resultConvertYonaTX.first){
            ...
            if (fLogEvents && !fJustCheck)
            {
                ...
                for(size_t k = 0; k < resultConvertYonaTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{
                        ...
                        tx.GetHash(),
                        uint32_t(i),
                        resultConvertYonaTX.first[k].from(),
                        resultConvertYonaTX.first[k].to(),
                        ...
                        resultConvertYonaTX.first[k].getNVout(),
...
bool CChainState::ConnectTip(BlockValidationState& state, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions& disconnectpool)
{
    ...
    LogPrint(BCLog::BENCH, "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * MILLI, nTimeReadFromDisk * MICRO);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // yona
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // yona
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // yona
            globalState->setRootUTXO(oldHashUTXORoot); // yona
...
bool SignBlockHWI(std::shared_ptr<CBlock> pblock, CWallet& wallet, std::vector<unsigned char>& vchSig)
{
    ...
    YonaLedger &device = YonaLedger::instance();
...
bool SignBlockLedger(std::shared_ptr<CBlock> pblock, CWallet& wallet)
{
    ...
    {
        std::string errorMessage = YonaLedger::instance().errorMessage();
...
bool CheckBlock(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, CChainState& chainstate, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (!CheckBlockHeader(block, state, consensusParams, chainstate, fCheckPOW, true))
        return false;
...
bool BlockManager::AcceptBlockHeader(const CBlockHeader& block, BlockValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, CChainState& chainstate)
{
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        ...
        // Check block header
        if (!CheckBlockHeader(block, state, chainparams.GetConsensus(), chainstate, true, false))
...
bool ChainstateManager::ProcessNewBlockHeaders(const std::vector<CBlockHeader>& headers, BlockValidationState& state, const CChainParams& chainparams, const CBlockIndex** ppindex,  const CBlockIndex** pindexFirst)
{
    ...
    AssertLockNotHeld(cs_main);
    {
        ...
        for (size_t i = 0; i < headers.size(); ++i) {
            ...
            if (!fReindex && !fImporting && !ActiveChainstate().IsInitialBlockDownload() && header.IsProofOfStake() && setStakeSeen.count(std::make_pair(header.prevoutStake, header.nTime)) && !BlockIndex().count(header.GetIndexHash())) {
                // if it is the last header of the list
                if(i+1 == headers.size()) {
                    if(fInstantBan) {
                        // if we've seen a dupe stake header already in this list, then instaban
                        return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "dupe-stake", strprintf("%s: duplicate proof-of-stake instant ban (%s, %d) for header %s", __func__, header.prevoutStake.ToString(), header.nTime, header.GetIndexHash().ToString()));
                    } else {
                        // otherwise just reject the block until it is part of a longer list
                        return state.Invalid(BlockValidationResult::BLOCK_HEADER_REJECT, "dupe-stake", strprintf("%s: duplicate proof-of-stake (%s, %d) for header %s", __func__, header.prevoutStake.ToString(), header.nTime, header.GetIndexHash().ToString()));
                    }
                } else {
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for Yona PoS we need to sync this after every block to ensure txdb is populated for
...
CAmount GetTxGasFee(const CMutableTransaction& _tx, const CTxMemPool& mempool, CChainState& active_chainstate)
{
    ...
    if(tx.HasCreateOrCall())
    {
        ...
        YonaTxConverter convert(tx, active_chainstate, &mempool, NULL, NULL, contractflags);
